...
public class OAuth2AuthController {
    ...
    public void onSuccess(Authentication authentication, HttpServletResponse response) throws IOException {
        ...
        if (username == null || username.isBlank()) {
            response.sendRedirect(frontendOAuth2RedirectUrl + "?error=" +
                URLEncoder.encode("Kakao user info (email/id) missing", StandardCharsets.UTF_8));
            return;
        }
        ...
        // ✅ accessToken + username 전달
        String redirectUrl = frontendOAuth2RedirectUrl
